...
Мощный скрипт для комплексной валидации системы тем компонентов. Выполняет **7 типов проверок**:
...
4. **Использование базовых настроек темы**: Проверяет файлы `style.scss` на отсутствие хардкода (цвета, шрифты, отступы, тени)
...
7. **Отсутствие импортов тем в файлах стилей**: Проверяет, что файлы стилей компонентов не импортируют темы напрямую
...
10. **Валидация 7**: Проверяет отсутствие импортов тем в файлах стилей
11. **Отчет**: Выводит результаты всех семи проверок
...
📊 Итоговая статистика:
   Всего проверено: 21 тем (7 проверок на тему)
   ✅ Валидных проверок: 96
...
**Валидация 7 - Отсутствие импортов тем в файлах стилей:**
- **Импорты тем в стилях**: `@use './themes/light.scss'` в файле `ComponentName.scss`
...
- **24 тем** проверено (6 компонентов × 4 темы + 4 глобальных темы)
- **126 валидных проверки** (8 проверок на тему)
