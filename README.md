...
- **🧩 Elements** - `CardHeader`, `CardBody`, `CardFooter`, `CardPreview`, `UserAvatar`, `ThemeSelector` (базовые элементы с поддержкой тем)
- **🧱 Blocks** - `UserProfileCard` (составные компоненты среднего уровня)
- **🏗️ Containers** - `BaseContainer` (контейнеры для управления темами)
...
- `ThemeSelector` - переключатель тем (элемент интерфейса)
